...
class CreateProcedureTest(unittest.TestCase):
    ...
    def test_validate_correct_master_time_slot_creation(self):
        """
        Testing that time slot is correctly created
        """
    ...
    def test_validate_blank_master_input(self):
        """
        Testing errors on blank master_id input in time_slot creation
        """
        master_id = None
        time_from = datetime.datetime.strptime('12:00', '%H:%M').time()
        time_to = datetime.datetime.strptime('18:00', '%H:%M').time()
        date = datetime.datetime.strptime('1/07/2022', '%d/%m/%Y').date()
        result = master_time_slot.time_slot_validation(time_from, time_to, date, master_id)
        self.assertTrue(result)
        self.assertTrue("master_id" in result)
        self.assertEqual("Choose master first", result["master_id"])

    def test_validate_master_id_doesnt_exist(self):
        """
        Testing errors on input of non-existing master while creating a time slot
        """
        created_user = user.create_user("Derek", "Melendez", "622", "CLIENT", "4442126374")
        time_from = datetime.datetime.strptime('15:00', '%H:%M').time()
        time_to = datetime.datetime.strptime('20:00', '%H:%M').time()
        date = datetime.datetime.strptime('16/04/2021', '%d/%m/%Y').date()
        try:
            result = master_time_slot.time_slot_validation(time_from, time_to, date, created_user.id)
            self.assertTrue(result)
            self.assertTrue("master_id" in result)
            self.assertEqual("No such master", result["master_id"])
        finally:
            user.delete_user(created_user.id)

    def test_validate_for_past_date(self):
        """
        Validate errors on creating time slot for past date
        """
    ...
    def test_validate_time_slot_time_to_too_late(self):
        ...
        date = datetime.datetime.strptime('10/05/2022', '%d/%m/%Y').date()
        try:
            result = master_time_slot.time_slot_validation(time_from, time_to, date, master_id)
            self.assertTrue(result)
            self.assertTrue("time_to" in result)
